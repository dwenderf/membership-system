...
The *Apply database migrations* workflow applies immediately and unattended on push to `main` — there is no approval step to wait for. This used to be an after-the-fact click: the workflow paused on `main` for a required reviewer on the `production` GitHub Environment, once the push had already landed. That gate ran too late to matter: Vercel deploys the new application code immediately and unconditionally on push to `main`, independent of this workflow, so the old design left a real window — observed once at over five hours — where production served new code against old schema before anyone got around to approving. Neither Environment has a required reviewer now.

The human checkpoint moved earlier instead of disappearing: `.github/CODEOWNERS` names a reviewer for `supabase/migrations/`, so a promotion PR carrying a migration surfaces that in its Files Changed / reviewers panel — say in the PR description which file needs applying, so whoever looks knows what they're looking at. **This is a documented convention, not a GitHub-enforced block.** Branch protection does not require Code Owner review, so nothing stops a merge if it's skipped, and it can't be made a hard block on a single-maintainer repo anyway — GitHub never lets a PR author satisfy a required review on their own PR, including a Code Owner review, so requiring it here would deadlock every promotion this account opens. Reading the migration diff before merging is on whoever merges; nothing enforces that they did.

Do not suggest the SQL editor as a way to apply a migration: it runs the SQL without writing a row to `supabase_migrations.schema_migrations`, so the CLI still thinks the migration is pending and the drift check reports the database as behind. If it is ever used, it has to be followed by `supabase migration repair --status applied <version>`. The workflow runs after the push has landed and cannot fail or undo a merge: a red run means the code is on the branch and the migration is not applied.

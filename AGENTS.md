...
By a human, deliberately, through the *Apply database migrations* workflow. Do not suggest the SQL editor as an equivalent: it applies the SQL without writing a row to `supabase_migrations.schema_migrations`, so the CLI still thinks the migration is pending and the next `db push` (or the preflight check below) reports the database as behind. If it is ever used, it has to be followed by `supabase migration repair --status applied <version>`. Merging a PR that touches `supabase/migrations/` starts that workflow automatically, but the enforcement that a migration is actually applied before anything merges lives on the pull request itself — see the next paragraph — not in this post-merge run. The workflow runs after the push has landed and cannot fail or undo a merge: a red run means the code is on the branch and the migration is not applied. Say in the PR description which file needs applying, so whoever picks it up knows what to run.

*Migration applied to target database*, a required PR check, fails a pull request outright if its target branch's database is missing a migration this repo carries — so a migration has to be applied (via *Apply database migrations*, manually, before merging) or the PR can't merge at all. This is the actual enforcement (#290); a required reviewer on the `supabase-production` Environment is optional defense-in-depth on top of it, not load-bearing.

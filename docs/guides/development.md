...
Use the Supabase CLI's format — a 14-digit UTC timestamp, an underscore, then a
descriptive name:
```
...
**Examples:**
```
20260713143000_add_user_tags_column.sql
20260713143500_fix_payment_rls_policies.sql
20260714090000_refactor_membership_pricing.sql
20260714091500_enhance_xero_integration.sql
```

### Naming Guidelines

**Timestamp**: Always `YYYYMMDDHHMMSS`, in UTC
- Use the time you create the migration
- Anything else (for example the old `YYYY-MM-DD-name.sql` style) is silently
  ignored by `supabase db push` and `supabase migration list` — the CLI reports
  "up to date" against a database that has had nothing applied
- Migrations run in filename order, which the timestamp keeps chronological
...
- `supabase/schema.sql` is the current state of the database, and is what a
  fresh Supabase project is bootstrapped from
- Don't edit it directly — it's generated by concatenating `supabase/migrations/`
  in filename order
- After adding a migration, run `npm run schema:build` and commit the result;
  `npm run schema:check` fails if the two have drifted
- The migration history was collapsed into `20260907000000_baseline_schema.sql`
  on 2026-09-07 (a snapshot of production); the 140 dated files that preceded it
  live in git history

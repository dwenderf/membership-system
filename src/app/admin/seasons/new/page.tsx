...
export default function NewSeasonPage() {
  ...
  const seasonExists = existingSeasons.some(season => 
    season.type === formData.type && season.fiscal_year === formData.start_year
  )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const seasonData = {
        name: seasonName,
        type: formData.type,
        fiscal_year: formData.start_year,
          ...
          <div className="bg-white shadow rounded-lg">
            <form onSubmit={handleSubmit} className="space-y-6 p-6">
              ...
              <div className="flex justify-end space-x-3">
                ...
                >
                  {loading ? 'Creating...' : canCreateSeason ? 'Create Season' : 'Cannot Create Season'}
                </button>
              </div>
            </form>
          </div>

-- Remove processing status logic from RPC functions
-- The SELECT FOR UPDATE SKIP LOCKED is sufficient for preventing race conditions
-- Processing status adds complexity without solving the root cause
...
-- Recreate the invoice function without processing status
...
BEGIN
  ...
  -- SELECT FOR UPDATE SKIP LOCKED is sufficient for preventing race conditions
  FOR rec IN 
  ...
  LOOP
    -- Return the record with line items (no status change needed)
    RETURN QUERY
    SELECT 
      xi.id,
      xi.payment_id,
...
-- Recreate the payment function without processing status
...
BEGIN
  ...
  FOR rec IN 
    ...
    FOR UPDATE SKIP LOCKED
  LOOP
    -- Return the record (no status change needed)
    RETURN QUERY
    SELECT 
      xp.id,
      xp.xero_invoice_id,
...
COMMENT ON FUNCTION get_pending_xero_invoices_with_lock(INTEGER) IS 'Gets pending invoices with row-level locking using SELECT FOR UPDATE SKIP LOCKED. No status changes - just prevents race conditions.';
COMMENT ON FUNCTION get_pending_xero_payments_with_lock(INTEGER) IS 'Gets pending payments with row-level locking using SELECT FOR UPDATE SKIP LOCKED. No status changes - just prevents race conditions.'; 